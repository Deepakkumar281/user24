...
const Searchlocation = ({navigation, route}) => {
  ...
  const picklocatoin = route?.params?.location;
  console.log('🚀 ~ Searchlocation ~ picklocatoin:', picklocatoin);
  ...
  const handleModalClose = () => {
    ...
    console.log(destinationAddress, destinationLocation, '][d][;dld');
  ...
  const handleSuggestionSelect = async placeId => {
    ...
    try {
      const response = await axios.get(url);
      console.log('🚀 ~ handleSuggestionSelect ~ response:', response.data);
      if (response.data.status === 'OK') {
        const result = response.data.result;
        const location = {
        ...
        navigation.navigate('home', {destaddress: location});
        setDropdownVisible(false);
      }
    } catch (error) {
      console.error('Error fetching place details:', error);
